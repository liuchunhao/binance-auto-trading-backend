...
from config import BASE_URL, KEY, SECRET
...
# KEY = "w706rQMj7OUPAtMHuZtCv02odP8faed5AsZVABsRZd8OmXHdQWOLXrqcSopUXRDj"
# SECRET = "7Yc592SRaIiqXVah07A6FIXxNwwUG1ZTexg42ZM9SRry6iv8lAIbk2R2s9nQ0HAs"
...
def send_signed_request(http_method, url_path, payload={}):
    ...
    response = dispatch_request(http_method)(**params) # type: ignore
    print(response)
...
def send_public_request(url_path, payload={}):
    ...
    print("{}".format(url))
    response = dispatch_request("GET")(url=url)  # type: ignore
    return response.json()
...
def get_account_snapshot():
    ...
    for snapshot in response['snapshotVos']:
        ...
        print('update_time: {}'.format(timestamp))

        print('update_time > today ? {}'.format(timestamp > today))
